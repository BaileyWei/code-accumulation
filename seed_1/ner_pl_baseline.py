# done:
# huggingface trainer clip gradient in l2 norm by default
# clip gradients' maximum magnitude to <=0.5
# trainer = Trainer(gradient_clip_val=0.5, gradient_clip_algorithm="value")
# Todo: 1.adding more arguments to the huggingface TrainingArguments
# Todo: 2.implementing eval delay in new Trainer
...
from seed.utils.pl_modules import TokenPairNERPLModule
...
def pre_process(example):
    ...
    for sample in samples:
        ...
        # done: convert word level labels into token level labels
...
# remove the input_ids column
# train_dataset = train_dataset.remove_columns(['input_ids'])
# val_dataset = val_dataset.remove_columns(['input_ids'])
...
pl_model = TokenPairNERPLModule(training_args, tokenizer, model)

train_dataloader = torch.utils.data.DataLoader(
    train_dataset,
    batch_size=training_args.per_device_train_batch_size,
    num_workers=training_args.dataloader_num_workers,
    collate_fn=DataCollatorForEGPBaseline(
        tokenizer=tokenizer,
...
trainer.save_checkpoint("seed/ner_baseline/checkpoint-epoch-10", True)
# trainer.test(dataloaders=test_dataloader)
# trainer.save_checkpoint('paddlepaddle/baseline/checkpoint-epoch-20')
