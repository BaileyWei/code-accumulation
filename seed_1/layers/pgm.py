...
def _get_encoding_type_from_tag_vocab(tag_vocab: dict) -> str:
    ...
    #     return 'bioes'
    raise RuntimeError("encoding_type cannot be inferred automatically. "
...
class ConditionalRandomField(nn.Module):
    ...
    I copy code from lonePatient/pytorch_crf, and modify it to fit my project.
    """
    ...
    def _gold_score(self, logits, tags, mask):
        ...
        mask = mask.eq(True)
        flip_mask = mask.eq(False)
        # print(tags[:seq_len - 1], tags[1:])
        # print(tags.size(), self.trans_m.size())
    ...
    def viterbi_decode(self, logits, mask, unpad=False):
        ...
        # print('crf:', max_len, seq_len)
        # max_len = min(max_len, max(seq_len))  # 做一下截断，以防万一，所以不允许出现中间有mask的（本来CRF也处理不了中间有mask）
