...
class BERTNERDataset(Dataset):
	...
	def __init__(self, args,json_path, tokenizer: BertWordPieceTokenizer, max_length: int = 512, possible_only=False,
				 pad_to_maxlen=False):
	...
	def __getitem__(self, item):
		...
		context_tokens = tokenizer.encode_plus(context, return_offsets_mapping=True , add_special_tokens=True)
		# context_tokens = tokenizer(context)

		tokens = context_tokens['input_ids']  # subword index
		# type_ids = context_tokens.type_ids  # the split of two sentence on the subword-level, 0 for first sent, 1 for the second sent
		type_ids = context_tokens['attention_mask']
